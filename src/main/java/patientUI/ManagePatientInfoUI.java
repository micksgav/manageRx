...
public class ManagePatientInfoUI extends JFrame implements ActionListener {
	...
	public ManagePatientInfoUI(String title, Patient patient) {
		setTitle(title);
		Rectangle screenDims = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
				.getDefaultConfiguration().getBounds(); // dimensions of screen
		setSize(screenDims.width, screenDims.height);
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setLayout(new BorderLayout());
		this.patient = patient;
		//textFieldPadding = new Insets((int) (screenDims.height *0.15), (int) (screenDims.width *0.02), (int) (screenDims.height *0.1), (int) (screenDims.width *0.02));
		...
		mainPanel = new JPanel(new GridBagLayout()); // information about GridBagLayout from https://docs.oracle.com/javase/tutorial/uiswing/layout/gridbag.html
		...
		Border textFieldPadding = new EmptyBorder((int) (screenDims.height *0.01), (int) (screenDims.width *0.01), (int) (screenDims.height *0.01), (int) (screenDims.width *0.01));
		...
		dateOfBirthField = new JTextField(patient.getDateOfBirthMonth() + " " + patient.getDateOfBirthDay() + ", " + patient.getBirthYear());
		...
		// when connecting to backend, use a hashmap to connect company with insurance number
		...
		editRecord.setFocusPainted(false);
